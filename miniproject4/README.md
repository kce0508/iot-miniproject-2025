...
## 파이썬 AI(API 동작) + ASP.NET Core 연동 프로젝트

### Python

#### 파이썬 웹애플리케이션
1. dJango : 대규모 웹앱 개발시 사용 프레임워크. 구조화 잘 되어 있음. 무겁다
...
- `ASP.NET Core` 비어있음으로 생성. MVC로 생성 시 필요없는 파일이 다수 생성
- HTTPS를 선택 해제
...
- ultralytics : YOLO 이미지, 동영상 객체 탐지
    - ultralytics를 먼저 설치하면 Pytorch CPU버전이 설치
- Pytorch GPU 사용버전 설치(2.9GB)
 
```shell
> pip3 install torch torchvision --index-url https://downloadl.pytorch.org/whl/cu126
```
