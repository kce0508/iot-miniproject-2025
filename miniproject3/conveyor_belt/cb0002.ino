...
void setup() {
    ...
    digitalWrite(motorDirectionPin, HIGH);  // 회전방향 지정하기. 결선 상태에 따라 전진, 후진이 결정
    value = 80;                             // 초기 속도는 80!
    analogWrite(motorSpeedPin, value);      // 초기 속도 80으로 모터 작동 시작
}

void loop() {
    if (Serial.availble()) {                // 시리얼 모니터에 값이 입력되었다면
        ...
        Serial.println(value);              // value값을 시리얼 모니터로 확인
